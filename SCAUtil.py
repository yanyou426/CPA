...
def lazy_pearson(X: np.ndarray, Y: np.ndarray):
    from scipy.stats import pearsonr
    res = np.zeros(Y.shape[1])
    for i, each in enumerate(Y.T):
        # print(each.shape, X.shape)
        res[i] = pearsonr(X, each)[0]
    return res

# X is a row vector of N*1 dimension and Y is a N*M dimension matrix
# X is guess value (intermediates) and Y is trace points
# author : Yuhang Ji
